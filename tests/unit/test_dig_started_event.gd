## DigStartedEvent — the project's SECOND concrete [Event] subclass. GDD §11.1 (Layered Design,
## **binding**), §4.2 (the dig the event announces), §11.2, §11.3, §13.2 tier 1, §13.4, §13.6,
## §14 M2.
##
...
##         carries `site_created`: the SECOND digger joining an existing site is also a state
##         change, and §13.6 wants an event for EVERY state change.
##
...
## NO vocabulary — a concrete event owns its own name, and `dig_started` (like `turn_ended` before
## it) is not one of the nine names §11.1 lists. The payload shape is a §13.4 resolution lettered
## **(AY)** ((AU)/(AV)/(AW)/(AX) must NOT be renumbered):
##   (AY) DigStartedEvent carries `player_index` (who ordered the dig), `unit_id` (which unit was
##       assigned), `hex` (the target), `remaining_turns` (the site's remaining worker-turns AFTER
##       the assignment) and `site_created` (true when this order CREATED the site, false when it
##       joined an existing one). `to_dict()` calls the base FIRST so "type" stays the FIRST key,
##       then appends JSON-scalar keys in declaration order — the `Vector2i` becoming `hex_q` and
##       `hex_r` (§11.1 line 708's save is JSON).
##
...
##
## The source scans at the bottom are written FRESH for `scripts/sim/events/dig_started_event.gd`.
extends GutTest

const DIG_STARTED_PATH: String = "res://scripts/sim/events/dig_started_event.gd"
const EVENT_PATH: String = "res://scripts/core/event.gd"
const EVENT_BUS_PATH: String = "res://scripts/core/event_bus.gd"

## (AY) — the canonical key order of `to_dict()`: "type" FIRST, then the payload in declaration
...
## §4.1 "axial coordinates (q, r)" — the target hex the fixtures announce.
const SAMPLE_HEX: Vector2i = Vector2i(1, -1)
const OTHER_HEX: Vector2i = Vector2i(-2, 3)

## §4.2 line 185 "Hard Rock | 2" — the remaining-turn value the fixtures carry.
...
## The COMPLETE public surface of DigStartedEvent: one const, five payload fields, `to_dict`.
...
const REQUIRED_PUBLIC_VARS: Array[String] = [
	"player_index", "unit_id", "hex", "remaining_turns", "site_created"
]
...
## §11.1 — DigStartedEvent IS an [Event]: the bus dispatches by `type`, so the subclass must carry
## the base's write-once type field. Asserted through the script's base script, never `is Node`.
func test_dig_started_event_is_an_event() -> void:
	var event: DigStartedEvent = _event(true)
	...
	assert_not_null(base_script, "§11.1: DigStartedEvent extends the Event base")
	if base_script == null:
		return
	assert_eq(
		base_script.resource_path,
		EVENT_PATH,
		"§11.1: DigStartedEvent must extend %s so the EventBus can dispatch it" % EVENT_PATH
	)


## §11.1 / M0-T3 (f) — the type name is exactly `dig_started`, declared once as the event's OWN
## constant, and the instance's INHERITED `type` field carries it (the constructor must call
## `super(TYPE_NAME)`).
func test_the_type_name_is_dig_started() -> void:
	assert_eq(
		DigStartedEvent.TYPE_NAME,
		&"dig_started",
		"§11.1: the event's type is exactly \"dig_started\""
	)
	assert_eq(
		typeof(DigStartedEvent.TYPE_NAME),
		TYPE_STRING_NAME,
...
## (AY)/§4.2 — the five payload fields are carried verbatim from the constructor, in the documented
## meaning: who ordered, which unit, the target hex, the site's remaining worker-turns AFTER the
## assignment, and whether this order CREATED the site.
func test_the_payload_is_carried_verbatim() -> void:
	var created: DigStartedEvent = DigStartedEvent.new(1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, true)
	assert_eq(created.player_index, 1, "(AY): player_index is the ordering player")
	assert_eq(created.unit_id, 7, "(AY): unit_id is the unit assigned to the site")
	assert_eq(created.hex, SAMPLE_HEX, "§4.1: hex is the target being dug")
	assert_eq(created.remaining_turns, HARD_ROCK_TURNS, "§4.2: worker-turns still to dig")
	assert_true(created.site_created, "(AY): this order CREATED the site")

	var joined: DigStartedEvent = DigStartedEvent.new(0, 3, OTHER_HEX, 1, false)
	assert_eq(joined.player_index, 0, "(AY): a second event carries its own payload")
	assert_eq(joined.unit_id, 3, "(AY): a second event carries its own payload")
	assert_eq(joined.hex, OTHER_HEX, "(AY): a second event carries its own payload")
	assert_eq(joined.remaining_turns, 1, "(AY): a second event carries its own payload")
	assert_false(joined.site_created, "§4.2 line 197: a SECOND digger joined an existing site")
...
## every logged replay. A raw `Vector2i` value would not survive §11.1 line 708's JSON round trip.
func test_to_dict_puts_type_first_then_the_payload_in_declaration_order() -> void:
	var serialized: Dictionary = DigStartedEvent.new(
		1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, true
	).to_dict()
	assert_eq(
		serialized.keys(),
		[
			"type", "player_index", "unit_id", "hex_q", "hex_r", "remaining_turns", "site_created"
		] as Array,
		"§11.1: to_dict() is {\"type\" FIRST, player_index, unit_id, hex_q, hex_r, "
			+ "remaining_turns, site_created}"
	)
	assert_eq(EXPECTED_DICT_KEYS.size(), 7, "sanity: the documented key order has seven entries")
	assert_eq(serialized.get("type"), "dig_started", "§11.1: the base renders type as a String")
	assert_eq(serialized.get("player_index"), 1, "(AY): the ordering player is serialized")
	assert_eq(serialized.get("unit_id"), 7, "(AY): the assigned unit is serialized")
	...
	assert_eq(serialized.get("remaining_turns"), HARD_ROCK_TURNS, "§4.2: the progress is serialized")
	assert_eq(serialized.get("site_created"), true, "(AY): the creation flag is serialized")


## §11.1 line 708 — EVERY serialized value is a JSON SCALAR: `save = JSON snapshot of GameState (+
## ruleset version + seed + command log)`. A `Vector2i` in the payload would survive `to_dict()`
## and die at the JSON boundary, which is exactly the kind of failure a later milestone inherits.
func test_every_serialized_value_is_a_json_scalar() -> void:
	var serialized: Dictionary = DigStartedEvent.new(0, 1, OTHER_HEX, 0, false).to_dict()
...
func test_to_dict_is_a_pure_function_of_every_payload_field() -> void:
	var base: Array[String] = _serialized_values(
		DigStartedEvent.new(1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, true)
	)
	assert_eq(
		base,
		_serialized_values(DigStartedEvent.new(1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, true)),
		"§11.1: equal payloads serialize identically"
	)
	var mutants: Array[DigStartedEvent] = [
		DigStartedEvent.new(0, 7, SAMPLE_HEX, HARD_ROCK_TURNS, true),
		DigStartedEvent.new(1, 8, SAMPLE_HEX, HARD_ROCK_TURNS, true),
		DigStartedEvent.new(1, 7, Vector2i(SAMPLE_HEX.x + 1, SAMPLE_HEX.y), HARD_ROCK_TURNS, true),
		DigStartedEvent.new(1, 7, Vector2i(SAMPLE_HEX.x, SAMPLE_HEX.y + 1), HARD_ROCK_TURNS, true),
		DigStartedEvent.new(1, 7, SAMPLE_HEX, 1, true),
		DigStartedEvent.new(1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, false),
	]
	for mutant: DigStartedEvent in mutants:
		assert_ne(
			base,
...
## infrastructure: a subscriber on `dig_started` receives the typed instance itself, not a copy.
func test_the_event_bus_dispatches_the_new_subclass_unchanged() -> void:
	...
	bus.subscribe(DigStartedEvent.TYPE_NAME, handler.on_event)
	var event: DigStartedEvent = _event(true)
	bus.emit(event)
	assert_eq(handler.calls, 1, "§11.1: a subscriber on dig_started is delivered the event")
	assert_true(
		is_same(handler.last_event, event), "§11.1: the bus delivers the very instance emitted"
	)


## M0-T3 (f) — dispatch is BY TYPE: a subscriber on `turn_ended` is not disturbed by a
## `dig_started`, and vice versa. Two concrete events now share the bus and must not leak into
## each other's subscriptions.
func test_dispatch_is_by_type_and_the_two_concrete_events_do_not_cross() -> void:
	var bus: EventBus = EventBus.new()
	...
	assert_eq(dig_handler.calls, 1, "M0-T3 (f): the dig subscriber hears the dig event")
	assert_eq(turn_handler.calls, 0, "M0-T3 (f): the turn subscriber does NOT")
	bus.emit(TurnEndedEvent.new(0, 1, 1))
	assert_eq(dig_handler.calls, 1, "M0-T3 (f): the dig subscriber is not disturbed")
	assert_eq(turn_handler.calls, 1, "M0-T3 (f): the turn subscriber hears the turn event")


# =============================================================================================
# D. MECHANICAL SOURCE SCANS on scripts/sim/events/dig_started_event.gd
...
func test_source_contains_no_float_literal_and_no_float_type() -> void:
	var code: String = _code_text(DIG_STARTED_PATH)
	assert_false(code.is_empty(), "sanity: %s must be readable and non-empty" % DIG_STARTED_PATH)
	...
	assert_null(
		...
		"§11.1: integer math only — %s must contain no float literal (found %s)" % [
			DIG_STARTED_PATH, "" if hit == null else hit.get_string()
		]
	)
	assert_false(
		code.contains("float"),
		"§11.1: integer math only — %s must not use the float type" % DIG_STARTED_PATH
	)
	assert_false(
		code.contains("integer_division"),
		"M0-T4 (d): nothing here divides — %s must not suppress that warning" % DIG_STARTED_PATH
	)
...
## ABSENT so a second source of dig numbers cannot grow here), no `Object.hash()` override, and no
...
func test_source_is_payload_only() -> void:
	...
	for banned: String in ["RulesError", "RulesLoader", "DigRules", "DigSite", "GameState"]:
		assert_false(
			code.contains(banned),
			"M1-T9/(AY): an event is a payload — %s must not name %s" % [DIG_STARTED_PATH, banned]
		)
	assert_false(
	...
	assert_false(
		...
		"§11.1: an event is a payload — %s must not know which Command emitted it" % DIG_STARTED_PATH
	)
...
func test_the_event_infrastructure_declares_no_vocabulary() -> void:
	...
	for banned: String in ["dig_started", "DigStarted", "turn_ended", "TurnEnded"]:
		assert_false(
...
func test_source_carries_no_vocabulary_and_hard_codes_no_gdd_number() -> void:
	var lowered: String = _code_text(DIG_STARTED_PATH).to_lower()
	assert_false(lowered.is_empty(), "sanity: %s must be readable and non-empty" % DIG_STARTED_PATH)
	...
	for found: RegExMatch in numeral.search_all(code):
		assert_true(
			...
			"§13.6: only %s are permitted in %s — found \"%s\"" % [
				str(PERMITTED_NUMERALS), DIG_STARTED_PATH, found.get_string()
			]
		)
...
## `total_turns` the renderer does not need) fails too.
func test_public_api_is_exactly_the_documented_members() -> void:
	...
	_collect_public_functions(DIG_STARTED_PATH, public_functions, undocumented)

	var event: DigStartedEvent = _event(true)
	for required: String in REQUIRED_PUBLIC_FUNCTIONS:
		assert_true(
			public_functions.has(required),
			"§11.2: %s must be a public function of %s" % [required, DIG_STARTED_PATH]
		)
		assert_true(
			event.has_method(required),
			"§11.2: %s must be callable on a DigStartedEvent instance" % required
		)
	assert_eq(
		public_functions.size(),
		REQUIRED_PUBLIC_FUNCTIONS.size(),
		"§13.4: the public API of %s is exactly %s — found %s" % [
			DIG_STARTED_PATH, str(REQUIRED_PUBLIC_FUNCTIONS), str(public_functions)
		]
	)
	assert_eq(
		undocumented.size(),
	...
	assert_eq(
		_collect_public_vars(DIG_STARTED_PATH),
		REQUIRED_PUBLIC_VARS,
		"§13.4: the public FIELD surface of %s is exactly %s" % [
			DIG_STARTED_PATH, str(REQUIRED_PUBLIC_VARS)
		]
	)
	assert_eq(
		_collect_public_consts(DIG_STARTED_PATH),
		REQUIRED_PUBLIC_CONSTS,
		"§13.4: %s declares exactly one public const (its OWN type name)" % DIG_STARTED_PATH
	)


## S7 §11.1/§11.2 — DigStartedEvent is a pure [RefCounted] in `scripts/sim/events/`: concrete
## events carry SIM payloads, so they live under `sim/` while `scripts/core/event.gd` stays pure
## infrastructure (the M0-T3 (f) / M1-T9 (AT) precedent, and the `turn_ended_event.gd` shape).
## Asserted through get_class() — `is Node` would be a PARSE ERROR here.
func test_dig_started_event_is_a_pure_refcounted_in_scripts_sim_events() -> void:
	assert_true(
		FileAccess.file_exists(DIG_STARTED_PATH),
		"§11.2: DigStartedEvent belongs at %s" % DIG_STARTED_PATH
	)
	assert_eq(
		_event(true).get_class(),
		"RefCounted",
		"§11.1: DigStartedEvent must be a pure RefCounted"
	)
...
## A fixture event: player 1 assigns unit 7 to SAMPLE_HEX with §4.2's Hard Rock time remaining.
func _event(created: bool) -> DigStartedEvent:
	return DigStartedEvent.new(1, 7, SAMPLE_HEX, HARD_ROCK_TURNS, created)
...
func _serialized_values(event: DigStartedEvent) -> Array[String]:
